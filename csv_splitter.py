        ...
        try:
            ...
            for part in parts:
                if b'filename=' in part:
                    ...
                    value_start = part.find(b'\r\n\r\n') + 4
                    selected_columns = json.loads(part[value_start:].strip().decode())
            ...
            logging.debug(f"Rows per file: {rows_per_file}")
            logging.debug(f"Selected columns: {selected_columns}")

            processed_files = process_csv(file_content, output_prefix, rows_per_file, selected_columns)
            ...
            self.send_header('Content-Disposition', 'attachment; filename="processed_csv_files.zip"')
            self.end_headers()
...
if __name__ == '__main__':
    PORT = 8000
    server = ThreadedHTTPServer(("", PORT), CSVSplitterHandler)
